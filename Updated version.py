...
cx = [int(100 + i * (700 - 100) / 9) for i in range(10)]
cy = [int(100 + i * (600 - 100) / 9) for i in range(10)]
bub_spd = [random.randint(1, 2) for _ in range(10)]
bub_rad = [20 - num for num in bub_spd]
bub_stat = [random.choice([0, 1, 2]) for _ in range(10)]
...
def draw_bubbles():
    global cx, cy, bub_spd, bub_rad, bub_stat
    for i in range(len(bub_rad)):       
        if bub_stat[i] == 0:
            ...
            glColor3f(0,0,1)
        mid_circle(cx[i], cy[i], bub_rad[i])
    glColor3f(1,1,1)
...
def re_start():
    ...
    shooter1_shift, shooter2_shift = 0, 0
...
def bubl_move():
    global bub_spd, bub_rad, cx, cy

    
    for i in range(len(bub_spd)):
        bub_collision1()
        bub_collision2()
        if bub_spd[i] < 0:  # Moving up
            if cy[i] - bub_rad[i] > 0:
                cy[i] += bub_spd[i]
            else:  # Hits the top edge
                bub_spd[i] = -bub_spd[i]
        
        elif bub_spd[i] > 0:  # Moving down
            if cy[i] + bub_rad[i] < 630:
                cy[i] += bub_spd[i]
            else:  # Hits the bottom edge
                bub_spd[i] = -bub_spd[i]
                
def bub_collision1():
    global shotBubble1_cy, shotBubble1_cx, shot1_stat, bub_rad, bub_spd, bub_rad, bub_stat, cx, cy, shotBubble_r
    global shooter1_cx, shooter1_cy, shotBubble1_cx, shotBubble1_cy, shot1_stat, shot2_stat, shotBubble_r
    global shooter2_cx, shooter2_cy, shotBubble2_cx, shotBubble2_cy, shot2_stat
    global score1, score2, shooter1_mode
    global shooter1_shift, shooter2_shift, shooter_r, shooter_s
    ...
    for j in range(len(c2x)):
        if shot1_stat[j] in [0, 1, 2]:
            sleft, sright = c2x[j] - r2, c2x[j] + r2
            sup, sdown = c2y[j] + r2, c2y[j] - r2
            for i in range(len(bub_stat)):
                bubdown, bubup = (
                cy[i] - bub_rad[i],
                cy[i] + bub_rad[i],
                )

                bubleft = cx[i] - bub_rad[i]
                if bubleft <= sright and shot1_stat[j] in [0,1,2]:
                    if (bubup >= sup and sup >= bubdown) or (
                        bubup >= sdown and sdown >= bubdown
                        ):
                        if bub_stat[i]==shot1_stat[j]:
                            score1 += 1
                            shot1_stat[j]=3
                            bub_stat[i]=(bub_stat[i]+1)%3
                            print(f"Score: Player_1={score1}, Player_2={score2}" )
                        else:
                            shot1_stat[j]=3
                            score1 -= 1
    ...
    for j in range(len(c2x)):
        if shot2_stat[j] in [0, 1, 2]:
            ...
            for i in range(len(bub_stat)):
                ...
                bubright = cx[i] + bub_rad[i]
                if bubright >= sleft and shot2_stat[j] in [0,1,2]:
                    if (bubup >= sup and sup >= bubdown) or (
                        bubup >= sdown and sdown >= bubdown
                        ):
                        if bub_stat[i]==shot2_stat[j]:
                            score2 += 1
                            shot2_stat[j]=3
                            bub_stat[i]=(bub_stat[i]+1)%3
                            print(f"Score: Player_1={score1}, Player_2={score2}" )
